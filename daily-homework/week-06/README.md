# 第 6 周：Next.js API route 与前后端连接

时间：2026-06-29 到 2026-07-05

主交付物：AI 学习助手 mock 全流程：前端提交到 API route，再返回总结；Java 副线把练习整理成可测试 service。

科班基础嵌入：HTTP 方法、状态码、JSON、CORS 基础概念；把 Python 函数边界过渡成 Java 类、异常和 service 边界。

## 每日入口

- [2026-06-29 周一：创建 summarize API route](./day-01.md)
- [2026-06-30 周二：请求和响应结构](./day-02.md)
- [2026-07-01 周三：输入校验和错误状态码](./day-03.md)
- [2026-07-02 周四：前端 fetch 接 API](./day-04.md)
- [2026-07-03 周五：画 HTTP 数据流](./day-05.md)
- [2026-07-04 周六：手动测试和小修](./day-06.md)
- [2026-07-05 周日：第六周复盘](./day-07.md)

## 本周最低验收

- AI 学习助手 mock 全流程：前端提交到 API route，再返回总结。
- Java 副线只做 2 个触点：service 封装 + JUnit 正常/异常测试。
- 至少一次 Codex 作业批改。
- 至少更新一次学习日志和追踪表。
- 周日写周复盘。
