...
今日主问题：页面怎样准确表达“尚未请求、正在等待、成功、失败”四种情况？

预计投入：2-3 小时。理解状态模型约 40%，编码、失败验证和迁移约 60%。Java 副线最多 45-60 分钟。

## 0. 今天在项目中的位置

- 昨天：页面可以发起异步 mock 总结，也能制造成功和失败。
- 今天：让用户看懂等待和失败，并避免状态互相冲突。
- 明天：把类型、状态和异步流程作为一个整体进行综合复盘。
...
今天可以使用多个 state 实现，但必须保证展示逻辑不矛盾。以后再讨论 reducer 或正式状态机。
...
为昨日 `loadGreeting` 实验增加 `loading` 和 `error`：
...
明天第一步：关闭代码，用纸或 Markdown 画出“输入 → state → async summarize → success/error UI”的完整流程。
