...
> 适用卡点：当本周主问题需要“TypeScript 基础类型”相关能力、实验或证据时选择。
> 使用规则：这是一张可选任务卡，没有指定日期或前后顺序；未选择或未完成不形成补债。

本周主题：TypeScript 与 Next.js 页面

任务问题：怎样在程序运行前发现摘要数据中的类型错误？

参考投入上限：2-3 小时。只做到足以推进本周主问题的深度，未选内容不延期、不补做。

## 0. 任务在项目中的位置

- 已知基础：你为 `/api/summarize` 设计了 request、success response 和 error response。
- 本任务：把 JSON 草稿变成 TypeScript 能检查的数据契约。
- 后续：把一个结果类型扩展为完整的 request/response interface，并讨论运行时校验。
...
4. 本任务不要求 Spring Boot；主线超时时可以只完成目录规划和方法签名。
...
- 算法：只有当前项目卡点或独立面试准备需要时才选择。
- 自由探索：按每周 4 小时独立安排，不作为本任务奖励或验收条件。
...
- 后续自然动作：打开本任务的类型文件，在不看答案的情况下写出 request 和 response 的字段草稿。
